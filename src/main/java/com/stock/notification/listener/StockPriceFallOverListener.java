...
@RabbitListener(queues = "stock.pricefallover.queue")
@Service
@Slf4j
public class StockPriceFallOverListener {
    ...
    public void listener(StockEntity stockEntity, Channel channel, Message message) throws IOException {
        log.info("收到股票价格跌超，准备通知用户" + stockEntity.getStockCode());
        try {
            //通知用户股票跌超
            userAlertService.notifyFallOver(stockEntity);
