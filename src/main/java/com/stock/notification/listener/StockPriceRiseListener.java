...
@RabbitListener(queues = "stock.pricerise.queue")
@Service
@Slf4j
public class StockPriceRiseListener {
    ...
    public void listener(StockEntity stockEntity, Channel channel, Message message) throws IOException {
        log.info("收到股票价格上涨，准备通知用户" + stockEntity.getStockCode());
        try {
            //通知用户股票上涨
            userAlertService.notifyRise(stockEntity);
